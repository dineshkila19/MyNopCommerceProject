 ...
 * 
 */
package com.mynop.actiondriver;
...
public class Action extends BaseClass  {
	...
	public static void click(WebDriver webDriver, WebElement locatorName) {
		Actions act = new Actions((WebDriver) webDriver);
		act.moveToElement(locatorName).click().build().perform();
	}
	
	public static void click(ThreadLocal<RemoteWebDriver> driver, WebElement locatorName) {
	...
	/**
	 * select value from DropDown by using selectByValue
	 * 
	 * @param locator     : Action to be performed on element
	 * 
	...
	public static boolean JSClick(WebDriver driver, WebElement ele) {
		...
		try {
			...
			//driver.executeAsyncScript("arguments[0].click();", ele);
			flag=true;
			...
			if(flag) {
				...
				System.out.println("Click Action is not Prerformed");
			}
		}
		return flag;
	}
	...
	public static String getCurrentTime() {
		String currentDate = new SimpleDateFormat("yyyy-MM-dd-hhmmss").format(new java.util.Date());
		return currentDate;
	}
